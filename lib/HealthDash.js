...
HealthDash = function (rawData, style, layout) {
	...
	with (L) {
		...
		with (L.leftBox) {
			L.list = new zLayout({
				x:getX(0, true), y:getY(0, true) + 60,
				width:innerWidth
			});
		};
		with (L.topBox) {
...
// 				"font-size":20
	...
	var MEASURE_MODES = {
		...
		"DeprivRatio":{
			label:"Ratio of deprivation quintile 5 vs 1"
		},
		"parity":{
			label:"Mean parity"
		},
	...
	dc.updateData = function () {
		...
		for (d = 1; d < 4; d++) { // For each dimension
			...
			dc.forSpaces({as:"all", mask:{d:d, a:dc.meta[d].tree[1]}}, function (x) { // Iterate through all the other dimensions
				...
				dc.setData(MEASURE, destX, out / sum);
			});
		};
					...
					return {
						content:
							...
							"</div><div class=subtitle>" +
								...
								dc.getTitle(3) + " - " + dc.getName({d:3, as:S.A}) +
							"</div>"
					};
				}
			},
				...
				curr:function (S, D, mode) {
					...
					return zo.extend({
						content:
							...
							"<td class=measure>" + zt.format(measure, Y.format[MEASURE]) + "</td>" +
				...
				curr:function (S, D, mode) {
					...
					return zo.extend({
						content:
							...
							"<td width=120><b>" + dc.getName({d:2, as:D.A}) + "</b></td>" +
