...
HealthDash = function (rawData, style, layout) {
	...
	// Layout
	var L = zt.getMaxLayout(800, 600, 0);
	with (L) {
		...
		with (L.leftBox) {
			L.list = new zLayout({
				x:getX(0, true), y:getY(0, true) + 40,
				width:innerWidth
			});
		};
		with (L.topBox) {
			L.profile = new zLayout({
				x:getX(0, true) + xGap, y:getY(0, true),
				width:innerWidth - 2 * xGap, height:innerHeight
			});
		};
		with (L.bottomBox) {
			L.tAxis = new zLayout({
				x:getX(0, true) + xGap, y:getY(1, true),
				width:innerWidth - xGap, height:14
			});
			L.dAxis = new zLayout({
				x:getX(1, true), y:getY(1, true),
...
// 				"font-size":20
			...
			gladwrap:{
				...
				fill:"magenta", opacity:0.0000001
			}
		}
	};
	...
	dc.updateData = function () {
		// Set mask
		var i, d, targPos, meta, root,
			mask = za.fill("all", rawdc.meta.length),
			dMap = [0, 1, PROFILE_PRI, PROFILE_SEC]; // Year, DHB, primary profile dimension, secondary profile dimension
		...
		if (MEASURE == "DeprivQ5") {
			...
			meta = ["Unknown", "1", "2", "3", "4", "5"]; // Unknown = 0
			...
			d = 5;
			meta = ["Unknown", "Under 16", "16-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45 and over"]; // Unknown = 0
		} else if (MEASURE == "1stTrimesterReg") {
			d = 6;
			targPos = rawdc.findMeta("name", {d:d}, "1");
			meta = ["Unknown", "1", "2", "3", "Postnatal"]; // Unknown = 0
		};
		// For each cell in dc, calculate data from rawdc
			...
			if (MEASURE != "count") {
				...
				// Special case: Deprivation quintile 1:5 ratio
					...
					if (x.as[d] == "all") {
						matches = [];
						x.as[d] = targPos[0];
						matches[0] = rawdc.calcData("sum", "val", x); // Number of quintile 5 in this cell
						x.as[d] = targPos[1];
		...
		for (d = 1; d < 4; d++) { // For each dimension
			...
			dc.forSpaces({as:"all", mask:{d:d, a:dc.meta[d].tree[1]}}, function (x) { // Iterate through all the other dimensions
				...
				if (MEASURE != "count") { // Don't do count
	...
	dc.getName = function (x) {
		var aPos = dc.asAPos(x);
		if (aPos == "all" || aPos.length == dc.getSize(x.d)) {
			return "All " + dc.getTitle(x.d);
		} else {
			return dc.getMeta("name", x);
		};
	};

	var TITLES = ["Year", "DHBs", "Facility Type", "Ethnicity", "Deprivation", "Maternal Age", "Registration Trimester", "Parity"];
	var PROFILE_MODES = [2, 3, 4, 5, 6, 7]; // Dimensions which are valid for profiles
	var INIT_YEAR = 1, PROFILE_PRI = 3, PROFILE_SEC = 4, MEASURE = "1stTrimesterReg";

	with (rawdc) {
		var i, options;
		for (i = 0; i < meta.length; i++) meta[i].title = TITLES[i];
		meta[2].colour = ["#253494", "#2c7fb8", "#41b6c4", "#a1dab4", "#e0ff98", "#c994c7"]; // Facility type
		meta[3].colour = ["#253494", "#2c7fb8", "#41b6c4", "#a1dab4", "#e0ff98", "#c994c7"]; // Ethnicity
		meta[4].colour = ["#253494", "#2c7fb8", "#41b6c4", "#a1dab4", "#e0ff98", "#c994c7"]; // Deprivation Deciles: 1, 2, 3, 4, 5, Unknown
		meta[5].colour = ["#253494", "#2c7fb8", "#41b6c4", "#a1dab4", "#e0ff98", "#e0ff98", "#e0ff98","#e0ff98", "#e0ff98",  "#c994c7"]; // Maternal age
		meta[6].colour = ["#253494", "#2c7fb8", "#41b6c4", "#a1dab4", "#e0ff98", "#c994c7"]; // Registration trimester
		meta[7].colour = ["#253494", "#2c7fb8", "#41b6c4", "#a1dab4", "#e0ff98", "#c994c7"]; // Parity
		...
		$("#profilePri, #profileSec").html(options).change(function (a) {
			...
			profile.onInit();
			profile.updateData();
			profile.remove("lines", t, e, null, profile.K);
			profile.remove("yLabels", t, e, null, profile.K);
			profile.remove("xLabels", t, e, null, profile.K);
			profile.remove("title", t, e, null, profile.K);
			profile.remove("blocks", t, e, function () {
				profile.add("lines", t, e, null, profile.K);
				profile.add("blocks", t, e);
				profile.add("yLabels", t, e, null, profile.K);
				profile.add("xLabels", t, e, null, profile.K);
				profile.add("title", t, e, null, profile.K);
				list.refresh("all", t, e, null, profile.K);
			});
		});
		$("#measure").html(
			"<option value='count'>Count</option>" +
			"<option value='1stTrimesterReg'>% registered in first trimester</option>" +
			"<option value='DeprivQ5'>% in deprivation quintile 5</option>" +
			"<option value='DeprivRatio'>Ratio of deprivation quintile 5 vs 1</option>" +
			"<option value='age'>INCOMPLETE - Mean age</option>"
		).change(function (a) {
			var t = null, e = "<>";
	...
	var baseMouseEvents = function (D) {
		...
		getClosest = function (e, D, d, mask) {
			var S = D.parent,
				dist, out = {as:null, dist:D.Y.snap},
				mousePos = zt.getEventPosition(e),
				rPos = S.axes.t.pointToRPos(mousePos), // Find segment
				aPos = dc.asAPos({d:d, r:Math.floor(rPos)}), // Floor is important for getting the right point - don't use pointToAPos
				A = dc.addSpaces(S.A, {d:d, a:[aPos - 1, aPos, aPos + 1]}); // Do pair before and pair after (to account for steep slopes)
			// Find the distance from each segment to the mouse position
			dc.forSpaces({as:A, mask:mask}, function (x) {
				if (!zt.isBetween(x.as[d], 0, dc.getSize(d) - 2)) return; // Invalid segment (out of range)
				var A = [x.as, dc.addSpaces(x.as, {d:d, a:x.as[d] + 1})], // Segment is from the current point to the next
					line = [S.getPoint(A[0]), S.getPoint(A[1])], // Find position of segment
					dist = zp.distToLine(mousePos, line); // Find distance between mouse position and segment
				if (dist < out.dist) out = {as:A, dist:dist}; // If this segment is closer, make it the new target
			});
			// Calculate distance to each end of the segment
			if (out.as) {
				dist = [
					zp.dist(S.getPoint(out.as[0]), mousePos), // Prev to mouse
					zp.dist(S.getPoint(out.as[1]), mousePos), // Next to mouse
				];
				out.as = (dist[0] <= dist[1]) ? out.as[0] : out.as[1]; // Use aSpace of the closer end
			};
			return out.as;
		},
	...
	var G = new zGuide(),
		...
		tAxis = new zSlideAxis({
			...
			plan:{label:null, playButton:null, baseNotch:null}
		}),
		dAxis = new zAxis({
	...
	var list = new zSwarm({
		...
		plan:{
			...
			title:{
				...
				init:function (S, D, mode) {
					var xPos = L.list.left, yPos = L.list.top - 20, rotation = -30;
					return {
						measure:{
							type:"zText",
							layout:{x:xPos, y:yPos, rotation:rotation}
						},
						count:{
							type:"zText", text:"Count",
							layout:{x:xPos + 50, y:yPos, rotation:rotation}
						},
						dhb:{
							type:"zText", text:"DHB",
							layout:{x:xPos + 100, y:yPos, rotation:rotation}
						}
					};
				},
				curr:function (S, D, mode) {return {measure:{text:zt.titleCase(MEASURE)}}}
			},
			dividers:{
				type:"zMultiDrone", mask:"fixed",
				init:function (S, D, mode) {
				...
				curr:function (S, D, mode) {
					...
					return zo.extend({
						content:
							...
							"<td width=50>" + zt.format(measure, Y.format[MEASURE]) + "</td>" +
							"<td width=50>" + zt.format(count, Y.format.count) + "</td>" +
							"<td>" + dc.getName({d:1, as:D.A}) + "</td>" +
							"</tr></tbody></table>",
						layout:{y:yPos}
					}, D.Y[S.getState(D.A)]);
				}
	...
	var profile = new zSwarm({
		...
		onInit:function (A) {
			...
			S.axes.d.Y.label.format = Y.format[MEASURE];
		...
		plan:{
			background:{
				...
				init:function (S, D, mode) {
					return {
						layout:{anchor:S.L, width:S.L.width, height:L.height},
						mouseEvents:baseMouseEvents
					};
				}
			},
			title:{
				...
				init:function (S, D, mode) {
					return {layout:S.L.getPoint(0, 0, true)};
				},
				curr:function (S, D, mode) {
					return {
						content:
							dc.getName({d:1, as:S.A}) + "<br>" +
							dc.getName({d:0, as:S.A}) + "<br>" +
							" (by " + dc.getTitle(2) + " & " + dc.getTitle(3) + ")"
					};
				}
			},
			yLabels:{
				...
				init:function (S, D, mode) {
					return {
						layout:{x:L.profile.right + 12, yAlign:"yCentre"},
						mouseEvents:baseMouseEvents
					};
				},
				curr:function (S, D, mode) {
					...
					return zo.extend({
						content:
							...
							"<td width=80><b>" + dc.getName({d:2, as:D.A}) + "</b></td>" +
							"<td>" + zt.format(dec || 0, Y.decFormat) + "</td>" +
							"</tr></tbody></table>",
						layout:{y:L.profile.getY(zo.mid(summed.start, summed.end))}
			...
			xLabels:{
				...
				curr:function (S, D, mode) {
					...
					return zo.extend({
						content:
							(summed.end == summed.start) ? "" :
							"<table class=xLabels width=20><tbody>" +
							"<tr><td><b>" + dc.getName({d:3, as:D.A}) + "</b></td></tr>" +
							"<tr><td>" + zt.format(dec || 0, Y.decFormat) + "</td></tr>" +
							"</tbody></table>",
						layout:{x:L.profile.getX(zo.mid(summed.start, summed.end)) - 10}
			...
			blocks:{
				...
				curr:function (S, D, mode) {
					...
					return zo.extend({
						layout:{
							anchor:L.profile.getPoint(sec.start, pri.start),
							width:(sec.end - sec.start) * L.profile.width,
							height:(pri.end - pri.start) * L.profile.height
						}
					}, D.Y[S.getState(D.A)]);
				}
			},
					...
					return {
						...
						mouseEvents:function (D) {
							...
							D.mousemove(function (e) {
								var A = getClosest(e, D, 0, ["mask", S.A[1], "mask", "mask"]);
								S.highlight(A);
							});
							D.click(function (e) {
								var A = getClosest(e, D, 0, ["mask", S.A[1], "mask", "mask"]);
