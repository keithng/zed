						...
						mouseEvents:function (D) {
							D.mousemove(function (e) {
								...
								dc.forSpaces({as:A}, function (x) {
									var as = [x.as, [x.as[0], x.as[1], x.as[2] + 1]];
									// Filter out invalid segments
									if (x.as[1] == ACTUAL && x.as[2] >= S.actual) return; // Line for actual, but out of actual range
									if (!zt.isBetween(as[0][2], 0, dc.getSize(2))) return; // Invalid segment (out of range)
									if (!zt.isBetween(as[1][2], 0, dc.getSize(2))) return; // Invalid segment (out of range)
									if (dc.getVal({as:as[0]}) == "") return; // Invalid segment (contains null values)
									if (dc.getVal({as:as[1]}) == "") return; // Invalid segment (contains null values)
									// Work out distance between mouse position and segment
									var s, area,
										points = [
											S.getPoint(as[0]), // Prev point
											S.getPoint(as[1]) // Next point
										],
										dist = [
											zp.dist(points[0], mousePos), // Prev to mouse
											zp.dist(points[1], mousePos), // Next to mouse
											zp.dist(points[0], points[1]) // Prev to next (base of triangle)
										],
										deg = [
											zp.matchDeg(zp.deg(points[0], points[1]) - zp.deg(points[0], mousePos)), // Prev
											zp.matchDeg(zp.deg(points[1], points[0]) - zp.deg(points[1], mousePos)) // Next
										];
									if (zt.isBetween(deg[0], 90, 270)) { // Shortest point is to prev
										out.push({as:as[0], dist:dist[0]});
									} else if (zt.isBetween(deg[1], 90, 270)) { // Shortest point is to next
										out.push({as:as[1], dist:dist[1]});
										...
										out.push({
											as:(dist[0] <= dist[1]) ? as[0] : as[1], // Use aSpace of the closer point
											dist:2 * area / dist[2] // Height of triangle
										});
									};
								});
								// Get best point
