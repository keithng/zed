...
function zLayout (l, parent) {
	...
	c.set = function (l) {
		...
		var cornersX = za.calcObjects(c.corners, "x", "sort"),
			cornersY = za.calcObjects(c.corners, "y", "sort");
...
function zDataCube (d) {
	...
	c.asAPos = function (x) {
		...
		if (x.isR) { // If it an rPos, convert to aPos
			...
			if (typeof x.p == "number" || x.p instanceof Array) return za.map(c.shown[x.d], x.p);
	...
	c.asRPos = function (x) {
		...
		if (!x.isR) { // If it an aPos, convert to rPos
			...
			if (typeof x.p == "number" || x.p instanceof Array) return za.unmap(c.shown[x.d], x.p);
	...
	c.forGenerations = function (d, f, reverse, skipFirst, skipLast) {
		var g, hierarchy = c.getMeta("hierarchy", {d:d});
		skipFirst = skipFirst || 0;
		skipLast = skipLast || 0;
		if (reverse) {
			for (g = hierarchy.length - 1 - skipFirst; g >= skipLast; g--) {
				f({d:d, a:hierarchy[g]});
			};
		} else {
			for (g = skipFirst; g < hierarchy.length - skipLast; g++) {
				f({d:d, a:hierarchy[g]});
			};
		};
	};
	...
	//  Tree nodes  //
	//////////////////
	c.makeHierarchy = function (d) {
		if (d != null) {
			var i, p, currGen = [], nextGen,
				childrenPos = [], leaves = [], generation = [], hierarchy = [],
				parentPos = c.meta[d].parentPos, // meta[d].parentPos is the position of the parent
			...
			if (parentName) for (parentPos = [], i = 0; i < c.getSize(d); i++) {
				if (parentName[i] == "root" || parentName[i] == null) parentPos[i] = null; // "root" and null mean this is a root node
				else parentPos[i] = c.shyPushMeta("name", d, parentName[i]); // Extend is a shy function - if the name already exists, it will only find rather than extend
			};
			// Create nodes and set parents
			if (parentPos) for (i = 0; i < c.getSize(d); i++) {
				p = parentPos[i];
				if (p == i) explode("zDataCube.makeHierarchy(): UH OH - " + c.getName({d:d, a:i}) + " is its own parent. Space-time continuum collapsing.");
		...
		} else for (d = 0; d < c.dLen; d++) c.makeHierarchy(d);
	};
	...
	c.addRootNode = function (d, name) {
		...
		c.setMeta(null, {d:d, a:aPos}, {
			...
			generation:0, leaves:c.calcMeta("leaves", {d:d, a:meta.hierarchy[0]}, "sum")
		});
	...
	c.findFamily = function (relationship, x, liveOnly) {
		...
		if (relationship == "all") {
			...
			out = c.getMeta("parentPos", x);
		} else if (relationship == "children") {
			out = c.getMeta("childrenPos", x);
		// Children of node, plus their children, plus their children... (liveOnly excludes all the ones with leaves of 0)
			...
			var i, parentPos, childrenPos,
				aPos = c.asAPos(x);
			if (relationship == "siblings") {
				parentPos = c.findParent(x); // Find its parent
				childrenPos = za.flatten(c.findChildren({d:x.d, a:parentPos})); // Its parents children are its siblings and itself
				out = za.subtract(childrenPos, aPos); // Remove itself from group
		...
		if (liveOnly && out instanceof Array) return za.asArray(c.findAllMeta("leaves", {d:x.d, a:out}, 0, ">")); // Return the same array structure, but with only live members
		else return out;
	};
	// Shortcuts
	...
	c.findRelationship = function (d, startAPos, endAPos) {
		...
		var i, pos,
			startToRoot = [startAPos].concat(c.findFamily("ancestors", {d:d, a:startAPos})),
			endToRoot = [endAPos].concat(c.findFamily("ancestors", {d:d, a:endAPos})).reverse();
	...
	c.calcHierarchyMeta = function (mode, type, d) {
		if (mode == "stacked") { // Stack is special because children must all fit within the range of the parent
			var hierarchy = c.meta[d].hierarchy;
			c.addCalcMeta("stacked", type, {d:d, a:hierarchy[0]}, 0); // Stack for root generation
...
// 						logger("zDataCube.calcHierarchyMeta(): UH OH - Range for " + c.getName(x) + " is " + zt.asString(oldVal) + ", but its children is " + zt.asString(newVal) + " {d:" + d + ", a:" + x.a + ", type:" + type + "}.");
// 					};
				});
			}, false, 0, 1);
...
// 						logger("zDataCube.calcHierarchyMeta(): UH OH - Value for " + c.getName(x) + " is " + oldVal + ", but the " + mode + " of its children is " + newVal + " {d:" + d + ", a:" + x.a + ", type:" + type + "}.");
// 					};
					return newVal;
				});
			}, true, 1, 0);
...
// 						if (zt.round(newVal.end, 10) != zt.round(oldVal.end, 10)) {
// 							logger("zDataCube.calcHierarchyMeta(): UH OH - Range for node " + zt.asString(x) + " is " + zt.asString(oldVal) + ", but its children is " + zt.asString(newVal) + " (type:" + type + ").");
// 						};
					});
				}, false, 0, 1);
			});
...
// 							logger("zDataCube.calcHierarchyData(): UH OH - Value for node " + x + " is " + oldVal + ", but the " + mode + " of its children is " + newVal + " (type:" + type + ").");
// 						};
						return newVal;
					})
				}, true, 1, 0);
	...
	c.importStrip = function (newStrip, noReset) {
		...
		c.makeHierarchy(); // Construct hierarchies if required
		return c;
	};
...
function zSwarmConstructor (c, orders, type) {
	zo.shyExtend(c, {
		...
		disaggregate:function (x, t, e, b, w) {
			...
			shown[x.d] = za.insert(shown[x.d], children, rPos); // Add children to datacube
...
// 			if (!za.contains(shown[x.d], parent)) shown[x.d] = za.insert(shown[x.d], parent, rPos); // Add parent to datacube it doesn't already exist
			c.updateData(t, e, null, c.K);
...
function zAxisConstructor (c, o) {
	zSwarmConstructor(c, zo.extend({
		...
		pointToRPos:function (x) {
			...
			return zt.mid(c.range[0] - c.Y.extension, c.range[1] + c.Y.extension, dec); // Then convert dec to rPos
		},
...
var zPlot = zp = {
	...
	completeBox:function (a) {
		...
		a.xCentre = (a.xCentre != null) ? a.xCentre : zt.mid(a.left, a.right);
		a.yCentre = (a.yCentre != null) ? a.yCentre : zt.mid(a.top, a.bottom);
...
var zArray = za = {
	...
	insert:function (a, val, pos) {
		return a.slice(0, pos).concat(val, a.slice(pos + 1));
	},
 	// Insert only if it doesn't already exist
	shyInsert:function (a, val, pos) {
		return (za.contains(a, val)) ? a : za.insert(a, val, pos);
	},
	...
	compareTests:function (mode, a) {
		...
		switch (mode) {
			...
			case "!=":
				return function (x, val) {return x != val};
			case "type":
				return function (x, val) {return zt.getType(x) == val};
			case "atPos":
	...
	get:function (a, v1, mode, v2) {
		var i = za.find(a, v1, mode, v2);
		return (i == null) ? null : a[i];
	},
	getLast:function (a, v1, mode, v2) {
		var i = za.findLast(a, v1, mode, v2);
		return (i == null) ? null : a[i];
	},
	getAll:function (a, v1, mode, v2) {
		return za.map(a, za.findAll(a, v1, mode, v2));
	},
	...
	removeAll:function (a, v1, mode, v2) {
		...
		for (i = 0; i < map.length; i++) out = out.concat(a.splice(map[i], 1));
		return out;
	},
	...
	findLastObject:function (a, type, v1, mode, v2) {return za.findLast(za.extract(a, type), v1, mode, v2)},
	...
	getLastObject:function (a, type, v1, mode, v2) {return a[za.findLastObject(a, type, v1, mode, v2)]},
	...
	removeLastObject:function (a, type, v1, mode, v2) {return a.splice(za.findLastObject(a, type, v1, mode, v2), 1)[0]},
	...
	percentile:function (a, dec) {
		...
		return zt.mid(a[Math.floor(n)], a[Math.ceil(n)], n - Math.floor(n));
	},
	...
	// As dec of the sum of the array
	dec:function (a) {
	...
	// Dec change from one element to the next
	...
	// Dec change from each element to the first element
...
var zObject = zo = {
	...
	equals:function (a, b, ordered) {
		...
		var i, p, aType = zt.getType(a), bType = zt.getType(b);
		if (aType != bType) return false; // Different type - fail
		if (aType == "object") {
			for (p in a) {
				if (!zo.equals(a[p], b[p])) return false; // Check each element pair
			};
			return true;
		} else if (aType == "array") {
			if (a.length != b.length) return false; // Different length to a
			if (ordered) { // Ordered comparisons of arrays
				for (i = 0; i < a.length; i++) {
					if (!zo.equals(a[i], b[i], true)) return false; // Check each element pair
				};
				return true;
	...
	//  Operators  //
	/////////////////
	// Particularly useful for points (e.g. {x:100, y:100})
	...
	mid:function (a, b, dec) {
		...
		if (bType == "object") {
			...
			return out;
		} else if (bType == "number" && zt.getType(a) == "number") return zt.mid(a, b, dec);
		else if (bType == "colour") return zt.getColour(dec, a, b);
		else return b; // Unknown type - push though without change
	},
	midStack:function (a) {return zt.mid(a.start, a.end)},
	round:function (a, dp) {
		var p, out = {};
...
var zTools = zt = {
	...
	getColour:function (dec, neutralColour, positiveColour, negativeColour) {
		...
		return zt.getHex({
			r:zt.mid(startColour.r, endColour.r, dec),
			g:zt.mid(startColour.g, endColour.g, dec),
			b:zt.mid(startColour.b, endColour.b, dec)});
	},
	// Get random colour
	...
	*/
	...
	scale:function (val, oldStart, oldEnd, newStart, newEnd) {
		...
		return zt.mid(newStart, newEnd, dec); // Map dec onto newStart and newEnd
	},
	// == n!
