...
function zDataCube (d) {
	...
	c.listSpaces = function (x) {
		var d, currS = [], out = [],
			...
			minPos = za.fill(0, c.dLen), maxPos = [],
			counter = minPos.slice(); // Counter is an dLen long array (e.g. [0, 0, 0]), each time counter is run, the right-most number gets +1, and if it exceeds maxVal at that position, it'll reset to minVal at that position and the number on its left will get +1
		...
		while (counter) {
			for (d = 0; d < c.dLen; d++) currS[d] = baseS[d][counter[d]]; // Converts counter to baseS val
	...
	c.makeHierarchy = function (d) {
		if (d != null) {
			...
			if (parentPos) for (i = 0; i < c.getSize(d); i++) {
				p = parentPos[i];
				if (p == i) explode("zDataCube.makeHierarchy(): UH OH - " + c.getMeta("name", {d:d, a:i}) + " is its own parent. Space-time continuum collapsing.");
	...
	c.findFamily = function (relationship, x, liveOnly) {
		...
		if (liveOnly && out instanceof Array) return c.findAllMeta("leaves", {d:x.d, a:out}, 0, ">"); // Return the same array structure, but with only live members
		else return out;
	};
	// Shortcuts
	...
	c.findRelationship = function (d, startAPos, endAPos) {
		...
		var i, pos,
			...
			endToRoot = za.reverse([endAPos].concat(c.findFamily("ancestors", {d:d, a:endAPos})));
		...
		if (mode == "stacked") { // Stack is special because children must all fit within the range of the parent
			...
			c.forGenerations(d, function (x) {
				c.forPos(x, function (x) {
					...
					if (zt.roundTo(newVal.end, 10) != zt.roundTo(oldVal.end, 10)) {
						logger("zDataCube.calcHierarchyMeta(): UH OH - Range for " + c.getMeta("name", x) + " is " + zt.asString(oldVal) + ", but its children is " + zt.asString(newVal) + " {d:" + d + ", a:" + x.a + ", type:" + type + "}.");
					};
				});
			}, false, 0, 1);
			...
			c.forGenerations(d, function (x) {
				c.forMeta(type, x, function (oldVal, x) {
					...
					if (oldVal != null && newVal != oldVal) {
						logger("zDataCube.calcHierarchyMeta(): UH OH - Value for " + c.getMeta("name", x) + " is " + oldVal + ", but the " + mode + " of its children is " + newVal + " {d:" + d + ", a:" + x.a + ", type:" + type + "}.");
					};
					return newVal;
				});
			}, true, 1, 0);
	...
	c.disaggregate = function (x) {
		var rPos = c.asAPos(x),
			childrenPos = za.asArray(c.findChildren(x));
		if (!childrenPos || !childrenPos.length) return; // Don't disaggregate if there are no children
		c.shown[x.d] = za.insert(c.shown[x.d], childrenPos, rPos); // Replace rPos with children - uses rPos because that's the position of aPos in .shown
	};
	// Removes an element and all its siblings, replaces it with its parents
	c.aggregate = function (x) {
		var rPos = c.asRPos(x),
			parentPos = c.findParent(x),
			familyPos = c.findChildren({d:x.d, a:parentPos});
		if (parentPos == null) return; // Don't aggregate if there is no parent
		c.shown[x.d][rPos] = parentPos; // Place parent at the aggregation point
		c.shown[x.d] = za.subtract(c.shown[x.d], familyPos);
	};
		...
		plan:{
			...
			label:{
				...
				init:function (S, A, D, Y) {
					...
					return {
						layer:1,
						text:S.dc.getMeta("name", {d:S.d, a:A[S.d]}),
			...
			baseNotch:{
				...
				curr:function (S, A, D, Y) {
					...
					return {
						...
						label:{text:"From\n" + S.dc.getMeta("name", x), layout:labelLayout}
					};
				}
			},
			currNotch:{
				...
				curr:function (S, A, D, Y) {
					...
					return {
						ring:{circle:{centre:anchor, radius:S.L.height / 2 - Y.ring["stroke-width"] / 2}},
						background:{points:flag},
						label:{text:S.dc.getMeta("name", x), layout:labelLayout}
					};
				}
			},
			playButton:{
...
var zObject = zo = {
	...
	midStack:function (a) {
		return zt.mid(0.5, a.start, a.end);
	}
};
