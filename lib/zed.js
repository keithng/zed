	...
	arc:function (arc) {
		...
		if (arc.innerRadius) {
			...
			for (i = 1; i <= segments; i++) {
				svg += zp.arcTo(arc.innerRadius, zp.addVector(arc.degStart + i * segmentDeg, arc.innerRadius, arc.centre)); // Split into segments (Minimum 90 degrees per part, as arcTo can't handle large circles)
			};
		...
		if (arc.outerRadius > arc.innerRadius) {
			...
			for (i = segments - 1; i >= 0; i--) {
				svg += zp.reverseArcTo(arc.outerRadius, zp.addVector(arc.degStart + i * segmentDeg, arc.outerRadius, arc.centre)); // Split into segments (Minimum 90 degrees per part, as arcTo can't handle large circles)
			};
		};
		return svg + "Z";
	},
	...
	return new zSmartAxis(zo.extend({
		...
		setBase:function (x, t, e, b, w) {
			...
			if (c.style.disableBackwards && c.baseSelected.r > c.selected.r) {
				c.selected = {d:c.d, a:c.baseSelected.a, r:c.baseSelected.r};
			};
		...
		axisSelect:function (x, t, e, b, w) {
			var c = this, thread = Math.round(Math.random() * 1000), // Current thread
				...
				plotSelected = function (rPos, forced) {
					...
					if (c.style.disableBackwards && c.baseSelected.r > c.selected.r) {
						c.baseSelected = {d:c.d, a:c.selected.a, r:c.selected.r};
					};
					c.G.scripting = false;
