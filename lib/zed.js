...
var startTime = $.now(),
	...
	zDebug = {
		...
		name:function (dc, A) {
			var i, out = [];
			for (i = 0; i < A.length; i++) out[i] = dc.getName({d:i, as:A});
			return out;
		},
...
var zPlot = zp = {
	...
	// Return closest distance between a and a line consisting of two points
	distToLine:function (a, line) {
		var s, area,
			dist = [
				zp.dist(line[0], a), // Start to point
				zp.dist(line[1], a), // End to point
				zp.dist(line[0], line[1]) // Start to end (base of triangle)
			],
			deg = [
				zp.matchDeg(zp.deg(line[0], line[1]) - zp.deg(line[0], a)), // Start
				zp.matchDeg(zp.deg(line[1], line[0]) - zp.deg(line[1], a)) // End
			];
		if (zt.isBetween(deg[0], 90, 270)) { // Shortest point is to start
			return dist[0];
		} else if (zt.isBetween(deg[1], 90, 270)) { // Shortest point is to next
			return dist[1];
	...
	sCurveTo:function (p, smooth, horizontal) {
		...
		var v = zp.vector(p[0], p[1]),
			d = v.dist / zp.cos(90 - v.deg) / smooth, // Secret sauce
			cp = [ // Control points
...
// 		if (l.xAlign != "xCentre") return zp.vflag(l, handleWidth, handleHeight);
...
function zLayout (layout, parent) {
	...
	c.zoom = function (targ, viewport) {
		...
		return c.set({
			...
			margin:c.margin * scale // Margin has to be scaled, or inner aspect ratio will fuck up
		});
	};
	c.unzoom = function () {
		if (c.prevLayout) {
	...
	c.getRect = function (innerBox) {
		return zo.round((innerBox) ? {
			x:c.left + c.margin, y:c.top + c.margin,
			width:Math.abs(c.innerWidth),
			height:Math.abs(c.innerHeight)
		} : {
			x:c.left, y:c.top,
...
function zSimpleDrone (c, orders, type) {
	zo.shyExtend(c, {
		...
		getPointAtDec:function (dec) {
			var length = c.raphObj.getTotalLength();
			return c.raphObj.getPointAtLength(length * dec);
		},
		////////////////////
		//  Add / Remove  //
		////////////////////
...
function zGuide () {
	...
	c.forget = function () {
		c.bQueue = []; // Forget all previously queued actions
		c.stop();
	},
	c.stop = function () {
		c.animating = false;
...
function zText (orders, t, e, b, w) {
	...
	c.parseText = function (text) {
		...
		if (f.toCase) text =
			(f.toCase == "upper") ? text.toUpperCase() :
			(f.toCase == "lower") ? text.toLowerCase() :
			(f.toCase == "camel") ? zt.camelCase(text) :
			(f.toCase == "title") ? zt.titleCase(text) :
			text;
		return text;
	};
	...
	c.redraw = function (orders, t, e, b, w) {
		...
		orders = zo.shyExtend({ // WATCH OUT! Layout must NOT be stored in c.O, as it is a *derived* layout based on the size of the text. If it's stored in c.O, it would be treated as the *source* layout, and another derived layout would be produced from it. Use shyExtend or clone to make sure that c.O is not infected by layout.
			...
			y:c.L.trueAnchor.y + (textY - boxY) * c.L.height - (textY - 0.5) * (2 * c.L.margin + c.bBox.height || 0), // Text is always yCentred
	...
	c.add(PAPER.text(c.L.x, c.L.y, ""), c.O);
...
function zComplexDrone (c, orders, type) {
	zSimpleDrone(zo.shyExtend(c, {
		...
		// Brings parts toFront() base on their O.layer value
		toFront:function () {
			var i, p, layers = [],
				addToLayer = function (D) { // For each part, find the right layer and put them there
					var p, a = D.O.layer * 1; // Make sure it's a number - if it's a string, layers will be converted to an object, and the properties (i.e. The actual layers) will not be in order in IE and Firefox
			...
			c.forEach(addToLayer); // Sort parts into layers
			if (layers.length == 0) c.forEach("toFront"); // No layers defined, just toFront everything
			else {
				for (i = 0; i < layers.length; i++) if (layers[i]) {
					for (j = 0; j < layers[i].length; j++) layers[i][j].toFront(); // Bring drones in each layer toFront()
				};
			};
		},
		///////////////
...
// 	};
	...
	c.redraw = function (orders, t, e, b, w) {
		...
		if (orders.layout) {
			...
			zo.extend(c.O, { // Add straight to c.O (reset reverts to this, and if it's not updated properly, you will cry)
				position:"absolute",
				left:docPos.x + "px",
				top:docPos.y + "px",
...
function zGladwrap (orders) {
	var c = new zRectangle(zo.extend({
		base:{
			layer:99,
			snap:10, // Default snap-to range
			fill:"magenta",
			opacity:0
		}
	}, orders));
	// Note: Parent must have .getPoint() to function
	c.getClosestLine = function (event, xAxis, mask) {
		var S = c.parent, dc = S.dc, d = xAxis.d, // Assumes that xAxis is a smart axis, which is a pretty fool-proof assumption, I think, because it has to be a dimension associated with a line, which much have datapoints to be a line
			dist, out = {as:null, dist:c.O.snap}, // If nothing is closer than snap-to, then return null
			mousePos = zt.getEventPosition(event),
			rPos = xAxis.pointToRPos(mousePos), // Find segment
...
function zSwarmConstructor (c, orders, type) {
	zo.shyExtend(c, {
		...
		forDrones:function (x, f, v1, v2, v3, v4, v5, v6) {
			...
			if (typeof f == "string") {
				for (i = drones.length - 1; i >= 0; i--) { // Backwards so that remove functions work properly
					if (drones[i][f]) {
						if (drones[i].objClass == "zSwarm") drones[i][f]("swarm", v1, v2, v3, v4, v5, v6); // Swarms can be drones for other swarms. When swarms are controlling other swarms, they can only control them as a whole (hence the use of "all")
		...
		get:function (x) {
			...
			else if (x.role && x.d == null && x.as == null) return za.getAllObjects(c.drones, "role", x.role); // Role only
			var d, i, matches,
				space = c.dc.parseSpace(c.addDimension(x)),
				A = c.asASpace(x),
			...
			for (d = 0; d < A.length; d++) { // Use only active parts of the mask as filtering conditions
				...
				} else if (za.isArray(A[d]) && space[d] != "all") {
					matches = [];
		...
		layer:function () {
			...
			for (p in c.axes) c.axes[p].forDrones("all", addToLayer); // Sort axes drones into layers
		...
		addDrone:function (role, A, mode, t, e, b, w) {
			...
			D.currOrders.layer = c.baseLayer; // Inherit layer from parent
		...
		frameRedraw:function (x, dec) {
			c.forDrones(x, function (D) {
				...
				if (D.objClass == "zSwarm") D.frameRedraw("all", dec);
		...
		// Redraw x drones instantly
		instantRedraw:function (x) {
			c.plot(x);
			c.frameRedraw(x, 1);
		},
		...
		refresh:function (x, t, e, b, w) {
			...
			if (c.currDec == 1) {
				if (!mode.keepMouseEvents) c.stopMouseEvents("all");
			...
			c.G.redraw(t, e, function () {
				...
				if (!mode.keepMouseEvents) c.startMouseEvents("all");
				if (b) b();
				...
				curr:function (S, D, mode) {
					var x = {d:S.d, r:S.selected.r || 0},
						...
						flag = zp.flag({
							anchor:anchor,
							width:D.Y.width, height:D.Y.height,
							xAlign:D.Y.xAlign, yAlign:D.Y.yAlign
						}, D.Y.flagWidth, D.Y.flagHeight),
						labelLayout = {anchor:zo.mid(flag[0], flag[2]), xAlign:"xCentre", yAlign:"yCentre"};
					return {
						ring:{
