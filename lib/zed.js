...
// 		console.log(zt.asString(out));
// 		console.log(zt.asString(c.altListSpaces(x)));
...
// 		c.verifyTreeData(mode, type, d);
	...
	c.autoImport = function (rawData) {
		if (!rawData) return;
		else if (rawData.meta && rawData.data) c.importProtoCube(rawData); // Protocube
		else if (rawData instanceof Array) c.importDumb(rawData); // Dumb
	};
...
function zSwarmConstructor (c, orders, type) {
	zo.shyExtend(c, {
		...
		//  Magic sauce  //
		///////////////////
		// Change the aSpace for the swarm and all cells (d specifies the part of the space to change, and p specifies what to change it to, but every drone receives the change)
		...
		setSpace:function (x, t, e, b, w) {
			...
			if (typeof aPos == "number") {
				if (typeof c.aSpace[d] == "number") c.aSpace[d] = aPos; // Change aSpace for swarm if it's a valid candidate for setSpace (drones can be valid candidates even if the swarm is not, depending on the drone's mask)
				c.forRoles(function (role) {
					...
					if (typeof c.dc.addSpaces(c.aSpace, role.mask)[d] == "number") { // Change drone which are valid candidates (i.e. If their role.mask[d] is a number, or if role.mask[d] is "mask" and c.R[d] is a number)
						c.forDrones(role.name, function (D) {D.A[d] = aPos});
					};
				});
			} else if (za.isArray(aPos)) {
				if (za.isArray(c.aSpace[d]) || c.aSpace[d] == "all") {
					c.aSpace[d] = aPos.slice(); // Change aSpace for swarm - very important to use .slice(), or else all kinds of shit fuck up
					c.forRoles(function (role) {
				...
				} else logger(c.type + ".setSpace(): Can't change " + zt.asString(c.aSpace[d]) + " to " + zt.asString(aPos) + ".");
			} else logger(c.type + ".setSpace(): " + za.asString(x) + " contains aPos (" + za.asString(aPos) + "), but is neither an array nor a number.");
		},
		...
		addRole:function (role, aSpace, mode, t, e, b, w) {
			if (role.mask == "fixed") c.addDrone(role.name, "fixed", mode, t, e, null, w);
			else {
				var i, spaces = c.dc.listSpaces({as:aSpace, mask:{as:role.mask}});
		...
		add:function (x, t, e, b, w) {
			var role = c.getRole(x),
				...
				aSpace = c.dc.addSpaces(c.aSpace, (typeof x == "string") ? null : c.asASpace(x));
			mode.initialising = true;
			if (role) { // A role is specified
				role.ignore = false; // Cancel ignore flag
				c.addRole(role, aSpace, mode, t, e, null, w || c.K);
			} else if (x == "fixed") { // Fixed
				c.forRoles(function (role) {
					if (role.mask == "fixed") c.addRole(role, aSpace, mode, t, e, null, w || c.K);
				});
...
// 				for (i = za.min(newShown); i <= za.max(newShown); i += notchStep) c.trimmedShown.push(i); // Use first point as reference
	...
	// Initialise
	c.A = c.aSpace = za.fill("na", c.dc.dLen);
	c.A[c.d] = "all";
...
function zAxis (orders, t, e, b, w) {
	...
	zAxisConstructor(c, zo.extend({
		type:"zAxis",
		dc:new zDataCube(1).importDumb(), d:0, aSpace:["all"],
...
function zSwarm (orders, t, e, b, w) {
	zSwarmConstructor(this, zo.extend({aSpace:"all"}, orders), "zSwarm");
	// Initialise
