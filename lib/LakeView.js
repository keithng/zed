	...
	var swarm = new zSwarm({ // Resources
		...
		treemap:function (aPos, t, e, b, w) {
			var S = this,
				...
				currTime = dc.asAPos({d:1, a:S.aSpace[1]}),
		...
		plan:{
			title:{
				...
				init:function (S, A, D) {
					return {
						text:"Budget " + dc.getMeta("name", {d:1, a:S.aSpace[1] - 1}),
			...
			bar:{
				...
				curr:function (S, A, D) {
					var rPos = dc.asRPos({d:0, a:A[0]}),
						baseDec = dc.getData("valDecStacked", {as:[A[0], BASE_TIME]}),
						...
						baseVal = dc.getData("val", {as:[A[0], BASE_TIME]}),
						children = dc.findChildren({d:0, a:A[0]}),
						childrenVal = dc.getData("valChange", {as:[children, A[1]]}),
				...
				curr:function (S, A, D, Y) {
					...
					return {
						title:{
							text:currName,
							layout:{
								x:posL.centre.x, y:posL.top - Y.subTitle["font-size"] * 3,
						...
						subTitle:{
							text:
								...
								"",
							layout:{
								x:posL.centre.x, y:posL.top - Y.subTitle["font-size"] * 1.8,
								xAlign:"xCentre", yAlign:"xCentre"
							}
						},
						width:{
							text:
								"$" + zt.format(data.baseVal, Y.valFormat) + " \n" +
								"(" + zt.format(data.baseDec.end - data.baseDec.start, Y.decFormat) + " of Budget in " + baseYear + ")",
							layout:{
						...
						pos:{
							text:
								"$" + zt.format(data.posVal, Y.valFormat) + " of new funding",
							layout:{
						...
						neg:{
							text:
								"$" + zt.format(data.negVal, Y.valFormat) + " of cuts",
							layout:{
						...
						net:{
							text:
								...
								" of \n$" + zt.format(Math.abs(data.netVal), Y.valFormat) +
								" (" + zt.format(Math.abs(data.netVal / data.baseVal), Y.decFormat) + ")",
							layout:{
								x:posL.right + bigGap,
				...
				curr:function (S, A, D) {
					...
					return {
						...
						val:{
							type:"zTextBox",
							text:(
								(val > 0) ? "$" + zt.format(val, Y.valFormat) + " in " + year : "Cut") +
								" (" + ((change == val) ? "New" : ((change > 0) ? "up $" : "down $") + zt.format(Math.abs(change), Y.valFormat)) + ")",
