HotTree = function (rawData, style, layout) {
	...
	var swarm = new zSwarm({
		...
		dc:dc,
		aSpace:[dc.meta[0].tree[1], CURR_YEAR],
	...
	var tAxis = new zSmartAxis({layout:L.tAxis, style:Y.tAxis, d:1, dc:dc, plan:{axisTitle:null}}),
		...
		sideBar = new zSwarm({
			...
			updateData:function (t, e, b, w) {
				var S = this, i;
				S.vals = dc.getData("val", {as:S.aSpace});
			...
			plan:{
				...
				title:{
					...
					curr:function (S) {return {text:dc.getName({d:0, as:S.aSpace})}}
				},
				scale:{
					...
					curr:function (S, A, D, Y) {
						var baseVal = swarm.getBaseVal(S.aSpace[0]),
							currVal = swarm.getCurrVal(S.aSpace[0]),
				...
				description:{
					...
					curr:function (S, A, D, Y) {
						var i, name, prevVal, currVal, content, vals = [],
							description = dc.getMeta("description", {d:0, as:S.aSpace}),
							children = dc.findChildren({d:0, as:S.aSpace}),
