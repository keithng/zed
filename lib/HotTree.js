...
HotTree = function (rawData, style, layout) {
	...
	// Layout
	var L = zt.getMaxLayout(1024, 768, 60);
	with (L) {
	...
	var Y = {
		...
		sideBar:{
			...
			description:{
				...
				decFormat:{base:"decFormat", forceSign:true, wordSign:false}
			},
			line:{
				stroke:"#667",
				"stroke-width":2
						...
						var aPos = S.highlighted,
							...
							decChange = currVal / prevVal - 1,
 							text = zt.format(currVal, Y.valFormat) +
								...
								(!prevVal) ? "\n(new)" :
								(decChange == 0) ? "\n(no change)" :
								"\n(" + zt.format(decChange, Y.decFormat) + ")");
						return {
							layer:1, text:text,
							layout:{
			...
			script:[
				{
					...
					layout:{x:L.centre.x, y:L.getY(0.3), autoHeight:true}
				},{
					...
					layout:{x:20, y:L.centre.y, xAlign:"left", yAlign:"bottom", autoHeight:true},
