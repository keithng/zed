HotTree = function (rawData, style, layout) {
	...
	with (dc) {
		...
		// Replace names (which must be unique for parentName-based tree to work) with displayNames, if it exists
		if (meta[0].displayName) for (i = 0; i < getSize(0); i++) {
			meta[0].name[i] = meta[0].displayName[i] || meta[0].name[i];
		};
	...
	var swarm = new zSwarm({
		...
		setHighlight:function (aPos, t, e, b, w) {
			...
			if (aPos != null) {
				...
				sideBar.focus(aPos, t, e, null, S.K);
				tooltipBox.showAt({
					text:dc.getName(x),
				...
				S.reset("all", t, e, b, w);
				sideBar.focus(S.selected.a, t, e, null, S.K);
		...
		plan:{
			block:{
				...
				init:function (S, A, D, Y) {
					var currVal = S.getCurrVal(A[0]),
						baseVal = S.getBaseVal(A[0]),
						change = (currVal / baseVal - 1) / Y.maxVal;
	...
	var tAxis = new zSmartAxis({layout:L.tAxis, style:Y.tAxis, d:1, dc:dc, plan:{axisTitle:null}}),
		...
		sideBar = new zSwarm({
			...
			dc:dc, rSpace:"all", // Can't use rPos properly here because swarm is hogging dc.shown
			focused:{d:0, a:ROOT},
			highlighted:null,
			focus:function (aPos, t, e, b, w) {
				this.focused = {d:0, a:aPos};
			...
			updateData:function (t, e, b, w) {
				var S = this, i;
				S.vals = dc.getData("val", {as:[S.focused.a, "all"]});
			...
			plan:{
				...
				title:{
					...
					curr:function (S) {return {text:dc.getName(S.focused)}}
				},
				scale:{
					...
					init:function (S, A, D, Y) {
						...
						for (i = 0; i < Y.maxNotches; i++) out.push(zo.clone(base, {
							fill:swarm.getColour(2 * i / (Y.maxNotches - 1) - 1),
					...
					curr:function (S, A, D, Y) {
						var baseVal = swarm.getBaseVal(S.focused.a),
							currVal = swarm.getCurrVal(S.focused.a),
				...
				description:{
					...
					curr:function (S, A, D, Y) {
						var i, name, prevVal, currVal, content, vals = [],
							description = dc.getMeta("description", S.focused),
							children = dc.findChildren(S.focused),
