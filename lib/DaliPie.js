DaliPie = function (rawData, style, layout) {
	...
	// Layout
	var L = zt.getMaxLayout(1200, 600, 50); // minWidth, minHeight, margin
	with (L) {
		L.tooltipBox = {
		...
		L.swarm = new zLayout({
			...
			width:Math.min(width - sideSwarm.width - 280, innerHeight * 0.8),
			height:Math.min(width - sideSwarm.width - 280, innerHeight * 0.8),
...
// 		}),
	...
	var swarm = new zSwarm({ // Resources
		...
		setRings:function () {
			var S = this,
				notchGap = zt.getClosestFactorOfTen((S.maxVal - S.minVal) / 2),
