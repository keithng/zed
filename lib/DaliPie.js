...
DaliPie = function (rawData, style, layout) {
	...
	var Y = {
		...
		format:{
			dec:{mode:"%", dp:1},
			weight:{mode:"%", dp:1},
			change:{base:"format.dec", forceSign:true},
			description:{base:"format.dec", noSign:true},
		},
		lines:{stroke:"#ddd"},
		axis:{
			maxNotches:6,
	...
	var swarm = new zSwarm({ // Resources
		...
		getTooltip:Y.customTooltip || function (A, D) {
			var S = this,
			...
			with (S.calcAll(A)) return "" +
				...
				"<br><b>" + zt.format(baseWeight, S.Y.format.weight) + "</b> of " + baseTime + " " + S.dc.getName({d:0, as:D.A}) +
				"<i>" + (
					(D.role == "bar") ?
						(!PAPER.isiPad) ?
			...
			title:{
				...
				curr:function (S, D, mode) {
					...
					return {
						...
						title:{
							text:category + " (" + zt.format(val, Y.format.change) + ")"
						},
						subtitle:{
		...
		plan:{
			description:{
				...
				curr:Y.customDescription || function (S, D, mode) {
					var data = swarm.calcAll(S.dc.addSpaces(S.A, S.axes.t.selected));
					with (data) return {content:
						...
						" and was " + zt.format(baseWeight, S.Y.format.weight) + " of the " + baseTime + " " + S.dc.getName({d:0, as:D.A}) + ".<p>" +
						S.dc.getName({d:0, as:D.A}) + (
