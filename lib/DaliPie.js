...
DaliPie = function (rawData, style, layout) {
	...
	with (L) {
		...
		var maxWidth = width - sideSwarm.width - 240;
		L.swarm = new zLayout({
			anchor:{
				x:zt.calcMid(sideSwarm.right, right, 0.5),
				y:L.sideSwarm.getY(0.6)
			},
 			width:Math.min(maxWidth, L.sideSwarm.height * 0.75),
 			height:Math.min(maxWidth, L.sideSwarm.height * 0.75),
	...
	var swarm = new zSwarm({ // Resources
		...
		getTooltipText:function (A, D) {
			...
			with (S.calcAll(A)) return "" +
				...
				"<b><i>" + (
					(D.role == "bar") ? "<p>Click to select this period" :
		...
		setHighlight:function (aPos, t, e, b, w) {
			...
			sideSwarm.setSpace({d:1, a:aPos});
			sideSwarm.refresh("all", t, e, null, S.K);
		},
		zoomIn:function (aPos, b) {
			var S = this;
			S.selected = {d:1, a:aPos};
			S.refresh("all", S.Y.spin.t, S.Y.spin.e, function () { // Spin!
				...
				S.refresh("all", S.Y.zoom.t, S.Y.zoom.e, function () { // Zoom!
					...
					S.setRings();
					S.refresh("all", S.Y.merge.t, S.Y.merge.e, b); // Break and iterate!
				});
			});
		},
		zoomOut:function (aPos, b) {
			var S = this, children = dc.findChildren(S.selected);
		...
		zoomTo:function (aPos, b) {
			if (aPos == null) { // If called with no arguments, zoom out
				if (this.selected.a == ROOT) return; // Can't zoom out from root, abort
				aPos = dc.findParent(this.selected);
			};
			...
			S.lock();
			S.remove("rings", 100, "<>");
			// Check zoomOutTab and zoomed state
			...
			title:{
				...
				init:function (S, D, mode) {
					return {
						...
						mouseEvents:function (D) {
							if (PAPER.isiPad) { // On an iPad
								D.click(function () { // Only has click interaction..
									if (S.highlighted != S.selected.a) { // On first click (not yet highlighted)
										S.setHighlight(S.selected.a); // Highlight
									} else S.zoomTo(); // On second click, zoom
								});
							} else { // On a non-iPad
								D.hover(function () {S.setHighlight(S.selected.a)}); // On hover, highlight
								D.click(function () {S.zoomTo()}); // On click, zoom out
							};
						}
					};
				},
			...
			slice:{
				...
				curr:function (S, D, mode) {
					...
					return {
						...
						mouseEvents:function (D) {
							if (state != "child") { // Inactive elements...
								D.click(function () {S.zoomTo()}); // Only has one action
							} else if (PAPER.isiPad) { // Active elements on an iPad...
								D.click(function (event) { // Only has click interaction..
									if (S.highlighted != D.A[1]) { // On first click (not yet highlighted)
										S.setHighlight(D.A[1]); // Highlight
							...
							} else { // Active elements on a non-iPad
								D.tooltip(null, tooltipBox, {width:L.tooltipBox.width}); // Hover tooltips
								D.hover(function () {S.setHighlight(D.A[1])}); // On hover, highlight
			...
			bar:{
				...
				init:function (S, D, mode) {
					return {
						...
						mouseEvents:function (D) {
							if (!PAPER.isiPad) D.tooltip(null, tooltipBox, {width:L.tooltipBox.width}); // No hover tooltips on an iPad
